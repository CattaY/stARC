...
def EWS_ttest(sdZ):
    p_values = np.zeros(len(sdZ))
    TS = [np.sqrt(len(sdZ)) * (np.mean(sdZ) - sdZ[i]) / (np.std(sdZ)) for i in range(len(sdZ))]
    for i in range(5, max(len(sdZ)-5, 6), 1):
        # t_statistic, p_values[i] = stats.ttest_1samp(TS[i], popmean=np.mean(TS[i-1], TS[i+1]))
        t_statistic, p_values[i] = stats.ttest_ind(TS[:i], TS[i:min(i+5, len(sdZ))], equal_var=False)
            # stats.ttest_1samp(TS[i:], popmean=np.mean(TS[:i]))
        if p_values[i] < 0.05: #p_values[i] < 0.02 and sdZ[i]>np.mean(sdZ) and sdZ[i] > sdZ[i-1]:
            break
    if i < len(sdZ)-6:
        EWS_idx = i
        pvalue = p_values[i]
    elif np.min(p_values) < 0.1:
        EWS_idx = np.argmin(p_values)
        pvalue = np.min(p_values)
    else:
        EWS_idx = 0
        pvalue = 1
    return EWS_idx, pvalue
...
def type_identification(devs1, devs2, EWS_idx, sigma=10):
    ...
    if abs(torch.median(torch.real(test1)-torch.real(test2))) <= 0.1 \
        ...
        return "NS"
            ...
            # and abs(torch.mean(torch.imag(test1)))<=0.1:
        return "Trans"
    elif torch.median(torch.real(test1)) <= -0.9:
            # and abs(torch.mean(torch.imag(test1)))<=0.1:
        return "PD"
...
def plot_perf_stARC(stARC_results, data_show, ews_point, ews_window,
    ...
    if 'devs1_str' in stARC_results.columns:
        ...
        # devs1 = torch.tensor([complex(item) for item in stARC_results['devs1_str']])
        # devs2 = torch.tensor([complex(item) for item in stARC_results['devs2_str']])
    else:
    ...
    # devs1_real = torch.real(devs1); devs1_imag = torch.imag(devs1)
    # devs2_real = torch.real(devs2); devs2_imag = torch.imag(devs2)
    ...
    if subplot[0] and arg['dataset'] == "Peter_lake":
        ...
        colors = ['#2372a9', '#2ca02c', '#9467bd', '#e377c2', '#17becf']  # blue, green, purple, pink, cran
        # colors = sns.color_palette("husl", len(data_show.columns))  # 或 "tab10", "Set2", "colorblind" 等
        # cmap = plt.get_cmap('Set2')  # 或 'tab20', 'Set2', 'Accent' 等
        # if arg['dataset'] == "Peter_lake":
        #     # 主坐标轴：绘制大数量级变量
        #     l1, = ax.plot(data_show.index, data_show['Chla'], color=colors[0], label='Chla')
        #     l2, = ax.plot(data_show.index, data_show['LMinnCPE'], color=colors[1], label='LMinnCPE')
        #     ax.set_ylabel('Chla\nLMinnCPE')
        #     # 副坐标轴：绘制小数量级变量
        #     ax2 = ax.twinx()
        #     l3, = ax2.plot(data_show.index, data_show['ZBgrav'], color=colors[2], label='ZBgrav')
        #     ax2.set_ylabel('ZBgrav', color=colors[2])
        # 分别在每个子图上画不同的线条，并统一 label
        axs[0].plot(data_show.index, data_show['Chla'], label='Chla', color=colors[0])
        axs[1].plot(data_show.index, data_show['LMinnCPE'], color=colors[1], label='LMinnCPE')
        axs[2].plot(data_show.index, data_show['ZBgrav'], color=colors[2], label='ZBgrav')
        axs[2].set_xlabel("DOY")
        for ax in axs:
            if bif_point_obsvt > 0:
                ax.axvspan(bif_point_obsvt, data_xlims[-1], color=color_map[3], alpha=color_map[4])
                ax.axvline(x=bif_point_obsvt, color=color_map[2], linestyle='--', linewidth=1.5,
                           label="observed bifurcation")
            if bif_point_para > 0:
                ax.axvline(x=bif_point_para, color=color_map[1], linestyle='--', linewidth=0.75,
                           label="parameter bifurcation")
                if bif_point_obsvt < 0:
                    ax.axvspan(bif_point_para, data_xlims[-1], color=color_map[3], alpha=color_map[4])
            if ews_window > 0:
                if bif_point_para < 0 and bif_point_obsvt < 0:
        ...
        if legend:
            ...
            fig.legend(loc="center right")  # 添加统一图例（放在画布外部右侧）
            # 自动调整布局，避免重叠
            plt.tight_layout(rect=[0, 0, 0.85, 1])  # 留出右边空间放 legend
        plt.savefig(path + '/%s data.svg' % arg['dataset'])
        plt.close()
    # 绘制 sd(Z)
    if subplot[1]:
    ...
    if arg["dataset"] in ['Dante_cave', 'Peter_lake',
                          'Africa_Humid_Period_shortcore', 'Africa_Humid_Period_longcore']:
        cmin = 0.25; cmax = 1
    ...
    if subplot[2]:
        plt.figure()
        if ews_window < 0:# or arg['dataset'] in ["Chick_heart"]:
            show_dev = len(devs1_real)
            Re, Im = devs1_real[:show_dev], devs1_imag[:show_dev]
        elif arg['dataset'] in ["Chick_heart"]:
            show_dev = len(devs1_real) #min(ews_window+30, len(devs1_real))
            Re, Im = devs1_real[:show_dev], devs1_imag[:show_dev]
        else:
            show_dev = ews_window
            Re, Im = devs1_real[:show_dev], devs1_imag[:show_dev]
